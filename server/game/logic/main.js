...
//Fool melonJS into running in node.js
var fs = require('fs');
...
(function($) {
    ...
    me.loader = (function() {
        ...
        function preloadImage(img, onload, onerror) {
            ...
            imgList[img.name] = {};
            imgList[img.name].src = img.src;

                  //use node.js module to find image size
            sizeOf(img.src, function (err, dimensions) {
                if (err) onerror();
                var dim = sizeOf(img.src);
                imgList[img.name].width = dim.width;
                imgList[img.name].height = dim.height
                onload();
            });
        };
        ...
        function preloadTMX(tmxData, onload, onerror) {
            ...
            if (tmxData.type === "tmx") {
                me.levelDirector.addTMXLevel(tmxData.name);
                        ...
                        fs.readFile(tmxData.src, 'utf8', function (err, data) {
                if (err) onerror();
                switch (format) {
                    case 'xml' :
                    case 'tmx' : {
                                    //use node.js module to parse
                        result = xmlParser.parseFromString(data);
                        format = 'xml';
                        break;
                    }
                    case 'json' : {
    ...
    me.TMXTileset = Object.extend({
        ...
        initFromImage: function (image, transparency) {
            ...
            if (transparency !== null && this.image) {
                // applyRGB Filter (return a context object) (disabled for node.js)
    ...
    me.levelDirector = (function() {
        ...
        obj.loadLevel = function(levelId) {
            ...
            if (levels[levelId] instanceof me.TMXTileMap) {
                ...
                // reset the gameObject Manager (just in case!) (disabled for node.js)
                //me.game.reset();
...
(function _init_ME() {
    ...
    // fake viewport with arbitrary dimensions for compatability
    me.game.viewport = new me.Viewport(0, 0, 100, 100, 1000, 1000);
